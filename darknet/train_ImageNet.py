...
def parse_args():
    ...
    parser.add_argument('--model-name', type=str, default='darknet53', help='model_name')
    parser.add_argument('--data-dir', type=str, default='~/.mxnet/datasets/imagenet', help='training and validation pictures to use.')
    ...
    parser.add_argument('-j', '--num-data-workers', dest='num_workers', default=4, type=int, help='number of preprocessing workers')
    ...
    parser.add_argument('--lr-mode', type=str, default='step', help='learning rate scheduler mode. options are step, poly and cosine.')
    parser.add_argument('--lr-decay', type=float, default=0.1, help='decay rate of learning rate. default is 0.1.')
    parser.add_argument('--lr-decay-period', type=int, default=0, help='interval for periodic learning rate decays. default is 0 to disable.')
    parser.add_argument('--lr-decay-epoch', type=str, default='200,250', help='epochs at which learning rate decays. default is 40,60.')
    parser.add_argument('--warmup-lr', type=float, default=0.0, help='starting warmup learning rate. default is 0.0.')
    parser.add_argument('--warmup-epochs', type=int, default=0, help='number of warmup epochs.')
    parser.add_argument('--logging-file', type=str, default='train_imagenet.log', help='name of training log file')
    parser.add_argument('--save-frequency', type=int, default=10, help='frequency of model saving.')
    ...
    parser.add_argument('--rec-train', type=str, default='~/.mxnet/datasets/imagenet/rec/train.rec', help='the training data')
    parser.add_argument('--rec-train-idx', type=str, default='~/.mxnet/datasets/imagenet/rec/train.idx', help='the index of training data')
    parser.add_argument('--rec-val', type=str, default='~/.mxnet/datasets/imagenet/rec/val.rec', help='the validation data')
    parser.add_argument('--rec-val-idx', type=str, default='~/.mxnet/datasets/imagenet/rec/val.idx', help='the index of validation data')
    parser.add_argument('--use-rec', action='store_true', help='use image record iter for data input. default is false.')

    parser.add_argument('--input-size', type=int, default=224, help='size of the input image size. default is 224')
...
def main():
    ...
    def train(ctx):
        ...
        for epoch in range(opt.resume_epoch, opt.num_epochs):
            ...
            for i, batch in enumerate(train_data):
                ...
                if opt.log_interval and not (i + 1) % opt.log_interval:
                    ...
                    logger.info('Epoch[%d/%d] Batch [%d/%d]\tSpeed: %f samples/sec\t%s=%f\tlr=%f' % (
                        epoch, opt.num_epochs, i, len(train_data), batch_size * opt.log_interval / (time.time() - btic),
            ...
            logger.info('[Epoch %d/%d] training: %s=%f' % (epoch, opt.num_epochs, train_metric_name, train_metric_score))
            logger.info('[Epoch %d/%d] speed: %d samples/sec\ttime cost: %f' % (epoch, opt.num_epochs, throughput, time.time() - tic))
            logger.info('[Epoch %d/%d] validation: err-top1=%f err-top5=%f' % (epoch, opt.num_epochs, err_top1_val, err_top5_val))
